...
// Order matters: scan current season first, then 2025-2026 archive, then older
const ARCHIVE_SEASONS = ["2025-2026", "2024-2025", "2023-2024", "2022-2023", "2021-2022"];
...
export async function GET(request: Request) {
    try {
        ...
        if (seasonParam && SEASON_FOLDERS[seasonParam]) {
            ...
            const merged = mergeMatches(existingMatches, taggedMatches);
        ...
        let pendingSeasons = [...ARCHIVE_SEASONS];

        if (cached?.scannedSeasons && cached.scannedSeasons.length > 0) {
            // New robust logic: explicit array of scanned seasons
            pendingSeasons = pendingSeasons.filter(seasonYear => !cached.scannedSeasons!.includes(seasonYear));
        } else if (existingMatches.length > 0) {
            // Fallback for older cache entries
            const cachedSeasons = new Set(existingMatches.map(m => m.sezon).filter(Boolean));
            pendingSeasons = pendingSeasons.filter(seasonYear => !cachedSeasons.has(seasonYear));
        }
        ...
        // Merge with existing cache (so we don't lose archive matches on refresh)
        const finalMerged = mergeMatches(existingMatches, taggedCurrentMatches);

        // Save to user cache
        await saveUserMatchesStore(session.userId, finalMerged, false, cached?.scannedSeasons || []).catch(e =>
...
function mergeMatches(existing: MatchData[], incoming: MatchData[]): MatchData[] {
...
function matchKey(m: MatchData): string {
    // Use multiple fields for dedup to avoid false matches
    const hakems = [...m.hakemler].sort().join(",");
    const masa = [...m.masa_gorevlileri].sort().join(",");
    return `${m.mac_adi}|${m.tarih}|${m.saat || ""}|${m.salon || ""}|${hakems}|${masa}`;
}
